...
// Macros
#define LED_LAT(val)  digitalWrite(LED_ShiftReg_LAT, val)
#define LED_SIN(val)  digitalWrite(LED_ShiftReg_SIN, val)
#define LED_SCLK(val)  digitalWrite(LED_ShiftReg_SCLK, val)
#define LED_BLANK(val)  digitalWrite(LED_ShiftReg_BLANK, val)
...
// Switch Pin Map:               {SW0, SW1, SW2, SW3, SW4, SW5, SW6, SW7, SW8, SW9, SW1
.000, SW11, SW12, SW13, SW14, SW15}
...
void writeLEDs(void)
{
 ...
 for(uint8_t u8_ledIndex = 0; u8_ledIndex < NUM_OF_LED_OUTS; u8_ledIndex++)
 {
   LED_SCLK(0);
   delayMicroseconds(1);
   LED_SIN(au8_LED_values[NUM_OF_LED_OUTS - u8_ledIndex - 1]);
   delayMicroseconds(1);
   LED_SCLK(1); 
 }
 LED_LAT(HIGH);
